...
alt_model = gen_model(args)
args.fusion_method = 'concat'
joint_model = gen_model(args)
...
modality_name_list = list(model['A'][KEY_ENCODERS].keys())
...
def forward(model, data_packet):
    ...
    alt_embedding_dict = forward_encoders(model["A"][KEY_ENCODERS], input_dict)
    joint_embedding_dict = forward_encoders(model["J"][KEY_ENCODERS], input_dict)

    alt_out_f = forward_fusion(model["A"][KEY_FUSION],
        alt_embedding_dict)
    
    joint_out_f = forward_fusion(model["J"][KEY_FUSION],
        joint_embedding_dict)
            
    final_out_f = (alt_out_f*len(alt_out_f) + joint_out_f)/(len(alt_out_f)+1)
        
    final_out_f_pred = softmax(final_out_f)     
    m.update(final_out_f_pred, labels_device)
